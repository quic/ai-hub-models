...
class OnnxSessionOptions:
    ...
    # If true, the ExecutableONNXModel API will create the context ONNX file for you and reuse it on future instantiations.
    ...
    #  However, since ExecutableONNXModel is sample code, we enable it by default to reduce potential user pitfalls.
...
class OnnxSessionTorchWrapper(ExecutableModelProtocol):
    ...
    def _prepare_inputs(self, inputs: dict[str, Any]) -> dict[str, np.ndarray]:
        ...
        for input_name, input_val in inputs.items():
            ...
            onnx_input_dtype, qdq_params = self.inputs[input_name]
    ...
    def _process_outputs(self, outputs: list[np.ndarray]) -> list[np.ndarray]:
        ...
        if self.quantize_io:
            ...
            for idx, (_, output_qdq_params) in enumerate(self.outputs.values()):
                output = outputs[idx]
