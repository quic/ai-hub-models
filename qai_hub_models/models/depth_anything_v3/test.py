...
def test_task() -> None:
    (_, _, height, width) = DepthAnythingV3.get_input_spec()["image"][0]
    app = DepthEstimationApp(DepthAnythingV3.from_pretrained(), height, width)
...
def test_trace() -> None:
    (_, _, height, width) = DepthAnythingV3.get_input_spec()["image"][0]
    traced_model = DepthAnythingV3.from_pretrained().convert_to_torchscript()
