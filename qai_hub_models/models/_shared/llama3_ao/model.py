...
from qai_hub_models.datasets.wikitext import WikiText, load_calibration_data
...
from qai_hub_models.models._shared.llama3_ao.app import RopeEmbedding
...
class Llama3Base_AIMETOnnx(LLM_AIMETOnnx):
    ...
    def get_calibration_data(
        ...
        return dataset_entries_to_dataloader(
            load_calibration_data(
                split=DatasetSplit.TRAIN,
                model=fp_model,
                device=torch.device("cuda" if torch.cuda.is_available() else "cpu"),
                num_samples=math.ceil(80000 / self.sequence_length),
                dataset_cls=WikiText,
            )
        )
