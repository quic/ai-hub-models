...
function createWindow () {
  ...
  mainWindow.loadFile('ai-solutions.html')
  console.log("Opened")
  // Open the DevTools.
...
app.whenReady().then(() => {
 ...
 execFile.stdout.on('data', (data) => {
  ...
  const processName = "server.exe"
  const command = "tasklist /fi \"imagename eq " + processName + "\"";

	// Run the command
	exec(command, (err, stdout, stderr) => {
	  if (err) {
		console.error(`Error executing the command: ${err.message}`);
		return;
	  }

	  // Check if the output contains the process name
	  const regex = new RegExp(processName, "i");
	  const isRunning = regex.test(stdout);

	  // Print the result
	  console.log(`${processName} is ${isRunning ? "running" : "not running"}`);
	});
 ...
 cppexecFile.stdout.on('data', (data) => {
  ...
  cppexecFile.on('close', (code) => {
    console.log(`snpe-sample child process exited with code ${code}`);
  });
