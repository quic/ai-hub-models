...
public class CameraFragment extends Fragment
{
    ...
    private void setUpCameraOutputs(int width, int height) {
        ...
        try {
            for (String cameraId : mCameraManager.getCameraIdList()) {
                ...
                Range<Integer>[] list = characteristics.get(CameraCharacteristics.CONTROL_AE_AVAILABLE_TARGET_FPS_RANGES);
                System.out.println("CameraID: "+cameraId);
                for(int i=0;i<list.length;i++)
                    System.out.println("FPS list: "+list[i]);
                ...
                System.out.println("shubham rotatedPreviewWidth, rotatedPreviewHeight, maxPreviewWidth, maxPreviewHeight" +
    ...
    private void createCameraPreviewSession() {
        try {
            ...
            Log.e(" ","createpreviewsession width="+mPreviewSize.getWidth()+" height="+mPreviewSize.getHeight());
            ...
            try {
                mCameraDevice.createCaptureSession(Collections.singletonList(surface), new CameraCapture(),
                        null);
            } catch (CameraAccessException e) {
                e.printStackTrace();
            }
    ...
    private class CameraSession extends android.hardware.camera2.CameraCaptureSession.CaptureCallback {
        ...
        public void onCaptureCompleted(@NonNull CameraCaptureSession session, @NonNull
            ...
            if (mNetworkLoaded) {
                ...
                float[] seg_map = new float[mBitmap.getWidth()*mBitmap.getHeight()];
                System.out.println("calling inference "+mBitmap.getHeight()+"::"+mBitmap.getWidth());
                Log.e("  ","calling inference "+mBitmap.getHeight()+"::"+mBitmap.getWidth());

                Trace.beginSection("snpeInference");
                float inferTime= mSnpeHelper.snpeInference(mBitmap, seg_map);
                Trace.endSection();
                ...
                int[] pix_array =new int[mBitmap.getWidth()*mBitmap.getHeight()];
                mBitmap.getPixels(pix_array,0,mBitmap.getWidth(),0,0,mBitmap.getWidth(),mBitmap.getHeight());

                for (int i = 0; i < mBitmap.getHeight(); i++) {
                    for (int j = 0; j < mBitmap.getWidth(); j++) {
                        float val=seg_map[i * mBitmap.getWidth() + j];
                        if (val != 0.0f) {

                            seg_map[i * mBitmap.getWidth() + j]=pix_array[i * mBitmap.getWidth() + j] & ColorArrayTemp[(int)(val)];

                        }
                        else {

                            seg_map[i * mBitmap.getWidth() + j]=pix_array[i * mBitmap.getWidth() + j];
                ...
                Log.e(" ","color array time="+total_time+" ms");


                if(inferTime!=-1.0f)
                {
                    Trace.beginSection("setting output");


                    mFragmentRender.set_map(seg_map,bitmap_width,bitmap_height,fps, inferTime);

                    Trace.endSection();
                }
                else
                    System.out.println("staus is -1");
            }
        }
    ...
    private boolean ensureNetCreated() {
        if (mSnpeHelper == null) {
            // load the neural network for image segmentation with SNPE
            mSnpeHelper = new SNPEHelper(getActivity().getApplication());

            mNetworkLoaded = mSnpeHelper.loadingMODELS(runtime_var, dlc_name_var);
