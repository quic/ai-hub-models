...
std::string build_network_BB(const uint8_t * dlc_buffer, const size_t dlc_size, const char runtime_arg, ModelName modelName);
bool SetAdspLibraryPath(std::string nativeLibPath);

bool executeDLC(cv::Mat &img, int orig_width, int orig_height, int &numberofhuman, std::vector<std::vector<float>> &BB_coords, std::vector<std::string> &BB_names, Model *modelobj);
