...
void YOLO_X_Model::preprocess(std::vector<float32_t> &dest_buffer, cv::Mat &img, std::vector<int> dims)
{
    LOGI("YOLO_X_Model preprocess");
    cv::Mat img640;
    //Resize and get the size from model itself (640x640 for YoloX)
    cv::resize(img,img640,cv::Size(dims[2],dims[1]),cv::INTER_LINEAR);
    ...
    for(int idx = 0; idx<lim; idx++)
        accumulator[idx]= img640.data[idx]*inputScale;
