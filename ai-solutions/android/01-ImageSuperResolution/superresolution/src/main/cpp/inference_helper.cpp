...
//Preprocessing and loading in application Input Buffer
bool loadInputUserBuffer(std::unordered_map<std::string, std::vector<float32_t>>& applicationBuffers,
    ...
    if (inputNames.size()) LOGI("Preprocessing and loading in application Input Buffer");
    ...
    for (size_t j = 0; j < inputNames.size(); j++) {
        ...
        if(bitWidth == 8 || bitWidth == 16) {
            ...
            modelobj->preprocess(applicationBuffers.at(name), img, dims);  //functions loads data in applicationBuffer
        }
    }
    return true;
}
