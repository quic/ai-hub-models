...
template <typename T> constexpr size_t get_safe_shift_mask()
{
    return CHAR_BIT * sizeof(promote_shift_operand_t<std::remove_cv_t<std::remove_reference_t<T>>>) - 1;
}

template <typename T, typename S, size_t mask = get_safe_shift_mask<T>()>
constexpr auto safe_lshift(T value, S shift_amount)
{
    static_assert(std::is_integral<T>::value && std::is_integral<S>::value,
                  "safe_lshift only makes sense for integral parameters");
    return static_cast<std::make_unsigned_t<T>>(value) << (static_cast<unsigned>(shift_amount) & mask);
}

template <typename T, typename S, size_t mask = get_safe_shift_mask<T>()>
constexpr auto safe_rshift(T value, S shift_amount)
{
    static_assert(std::is_integral<T>::value && std::is_integral<S>::value,
