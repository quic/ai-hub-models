...
class QnnSampleApp {
 public:
  ...
  StatusCode executeGraphs(float32_t* input_buffer,cv::Mat output_buffer[2],std::vector<size_t> output_dims[2]);
 ...
 private:
  ...
  std::string m_cachedBinaryPath = "libmodel-MiDAS_quant.serialized.bin";
