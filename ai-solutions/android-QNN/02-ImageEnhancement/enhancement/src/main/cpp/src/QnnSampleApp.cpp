...
sample_app::StatusCode sample_app::QnnSampleApp::executeGraphs(float* input_buffer,cv::Mat& output_buffer,std::vector<size_t> &output_dims, int finalOutputIndex) {
    auto returnStatus = StatusCode::SUCCESS;
...
//          &(outputs[outputIdx]), outputPaths, outputFile, outputBatchSize);
                ...
                Qnn_Tensor_t* output = &outputs[finalOutputIndex];
...
//                    outputPaths, fileName, dims, QNN_DATATYPE_FLOAT_32, bufferToWrite, outputBatchSize))
                ...
                __android_log_print(ANDROID_LOG_ERROR, "QNN ", "graphInfo.numOutputTensors = %d\n", graphInfo.numOutputTensors);
                ...
                for (size_t outputIdx = 0; outputIdx < graphInfo.numOutputTensors; outputIdx++) {
                    ...
                    if (nullptr != QNN_TENSOR_GET_NAME(outputs[outputIdx]) &&
                        ...
                        __android_log_print(ANDROID_LOG_ERROR, "QNN ", "%d: outputFilePrefix = %s\n", outputIdx, outputFilePrefix.c_str());
                    }

                    output_buffer  = cv::Mat(output_dims[1], output_dims[2], CV_32FC3, reinterpret_cast<float32_t*>(bufferToWrite));
